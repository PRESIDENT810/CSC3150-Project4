...
__global__ void mykernel(uchar *input, uchar *output) {
	...
	FS_node *root_node = new FS_node;

	FS_node *node_arr = new FS_node[50];
	root_node->child_node = node_arr;

	root_node->path[0] = '\0';

	fs_init(&root_fs, SUPERBLOCK_SIZE, FCB_SIZE, FCB_ENTRIES,
		VOLUME_SIZE, STORAGE_BLOCK_SIZE, MAX_FILENAME_SIZE,
		MAX_FILE_NUM, MAX_FILE_SIZE, FILE_BASE_ADDRESS);

	root_node->this_FS = &root_fs;
	root_fs.node_ptr = root_node;

	// user program the access pattern for testing file operations
	user_program(&root_fs, input, output);
}
